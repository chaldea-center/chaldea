...
(function () {
  ...
  async function timeout(promise, duration, debugName) {
    ...
    const _clock = new Promise((_, reject) => {
      timeoutId = setTimeout(() => {
        reject(
          new Error(`${debugName} took more than ${duration}ms to resolve. Moving on.`, {
            cause: timeout,
          })
        );
      }, duration);
  ...
  class FlutterServiceWorkerLoader {
    ...
    async _getNewServiceWorker(serviceWorkerRegistrationPromise) {
      ...
      if (!reg.active && (reg.installing || reg.waiting)) {
        ...
        // MOD 3: update indicator text
        let elements = document.getElementsByClassName("loading-hint");
        if (elements.length > 0) {
          elements[0].innerHTML = "Updating Chaldea...";
        }
  ...
  class FlutterEntrypointLoader {
    ...
    async loadEntrypoint(options) {
      // MOD 1: use hashed name
      const { entrypointUrl = window.entrypointUrl || "main.dart.js", onEntrypointLoaded } = options || {};
    ...
    _createScriptTag(url) {
      ...
      // MOD 2: remove indicator after loaded
      scriptTag.onload = () => {
