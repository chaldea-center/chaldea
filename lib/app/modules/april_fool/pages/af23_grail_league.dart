...
class FateGrailLeague extends StatefulWidget {
  const FateGrailLeague({super.key});

  @override
  State<FateGrailLeague> createState() => _FateGrailLeagueState();
}

class _FateGrailLeagueState extends State<FateGrailLeague> with AprilFoolPageMixin {
  @override
  final manifestUrl = 'https://static.atlasacademy.io/JP/External/FGL/manifest.json';

  @override
  Future<void> parseManifest(List<AAFileManifest> files, AprilFoolPageData data) async {
    data
      ..maxUserSvtCollectionNo = 375
      ..size = const Size(512, 724)
      ..iconAspectRatio = data.size.aspectRatio
      ..getFilename = (data) {
        return 'fgl-${data.curSvt?.id}'
    ...
    for (final file in files) {
      final m = RegExp(r'SaintGraph/card_sg_(\d+)').firstMatch(file.fileName)?.group(1);
    ...
    data.backgrounds = [
      for (int rarity = 1; rarity <= 5; rarity++) baseUri.resolve('Texture2D/bg_saintgraph_bg_$rarity.png').toString(),
    ];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: buildAppBar('Fate/Grail League'),
      body: Column(
        children: [
          Expanded(
            child: ListView(
