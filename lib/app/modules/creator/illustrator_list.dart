  ...
  void _parse() async {
    ...
    void _update<T>(Map<String, List<T>> map, String illust, T card) {
      ...
      for (final creator in creators) {
        map
            .putIfAbsent(Transl.illustratorNames(creator.trim()).l, () => [])
            .add(card);
      }
    }

    for (final svt in db2.gameData.servants.values) {
      _update(svtMap, svt.profile.illustrator, svt);
    }
    for (final ce in db2.gameData.craftEssences.values) {
      _update(craftMap, ce.profile.illustrator, ce);
    }
    for (final cc in db2.gameData.commandCodes.values) {
      _update(codeMap, cc.illustrator, cc);
    }

    illustrators = {...svtMap.keys, ...craftMap.keys, ...codeMap.keys}.toList();
  ...
  Widget listItemBuilder(String creator) {
    ...
    return SimpleAccordion(
      headerBuilder: (context, _) => ListTile(
        title: Text(Transl.illustratorNames(creator.trim()).l),
      ...
      contentBuilder: (context) {
        return Column(
          ...
          children: [
            ...
            if (craftMap[creator] != null) _cardGrid(craftMap[creator]!),
