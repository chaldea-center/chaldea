...
class BattleData {
  ...
  List<List<int>> getTraitsForFuncTargets(FuncTargetType targetType, BattleServantData self) {
    final List<BattleServantData> targets = [];
    ...
    final targetedAlly = getTargetedAlly(self);
    final targetedEnemy = getTargetedEnemy(self);
    ...
    switch (targetType) {
      ...
      case FuncTargetType.enemyOneNoTargetNoAction:
        final target = getServantData(self.getRevengeTargetUniqueId(), onFieldOnly: true);
        if (target != null) {
          targets.add(target);
        }
        break;
    ...
    return targets.map((e) => e.getTraits(addTraits: e.getBuffTraits()).toIntList()).toList();
  }
