...
class _MysticCodePageState extends State<MysticCodePage> {
  ...
  Widget build(BuildContext context) {
    ...
    _level = db.curUser.mysticCodes[_curCodeName]?.clamp2(1, 10) ?? 10;
    return Scaffold(
      appBar: AppBar(
  ...
  void _scrollTo(int dx) {
    ...
    _curIndex = (_curIndex + dx).clamp2(0, codes.length - 1);
    setState(() {
            ...
            texts: [
              ...
              for (int i = row * 5; i < row * 5 + 5; i++)
                i >= mysticCode.expPoints.length
                    ? '-'
                    : ((mysticCode.expPoints.getOrNull(i) ?? 0) -
          ...
          CustomTableRow.fromTexts(
            texts: [
              ...
              for (int i = row * 5; i < row * 5 + 5; i++)
                i >= mysticCode.expPoints.length
                    ? '-'
                    : mysticCode.expPoints[i].format(compact: false),
            ],
            defaults: TableCellData(maxLines: 1),
          ),
        ],
