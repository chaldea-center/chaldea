...
class _ExchangeTicketTabState extends State<ExchangeTicketTab> {
    ...
    return StreamBuilder<ItemStatistics>(
      ...
      builder: (context, snapshot) => ListView(
        children: divideTiles(
          tickets.map((ticket) {
            TextStyle plannedStyle = sum(db.curUser.events.exchangeTickets[ticket.month] ?? []) > 0
                ...
                Expanded(
                    ...
                    child: ListTile(
                      ...
                      title: AutoSizeText(ticket.month,
                          maxLines: 1,
                          maxFontSize: 16,
                          style: plannedStyle.copyWith(fontWeight: FontWeight.w600)),
  ...
  Widget buildTrailing(ExchangeTicket ticket, ItemStatistics statistics) {
    final monthPlan = db.curUser.events.exchangeTickets.putIfAbsent(ticket.month, () => [0, 0, 0]);
    ...
    for (var i = 0; i < 3; i++) {
      ...
      trailingItems.add(Row(
        ...
        children: <Widget>[
          GestureDetector(
            onTap: () =>
                SplitRoute.popAndPush(context, builder: (context) => ItemDetailPage(iconKey)),
                ...
                children: <Widget>[
                  ...
                  DefaultTextStyle(
                    ...
                    child: AutoSizeText(
                      ...
                      style: TextStyle(color: leftNum >= 0 ? Colors.grey : Colors.redAccent),
                    ),
                  )
                ],
              ),
              onPressed: () {
                Picker(
                  ...
                  onConfirm: (picker, values) {
                    ...
                    for (var j = 0; j < 3; j++) {
                      monthPlan[j] = min(monthPlan[j], ticket.days - sum(monthPlan.getRange(0, j)));
                    }
  ...
  Widget buildAll(List<ExchangeTicket> tickets, ItemStatistics statistics) {
    return StaggeredGridView.countBuilder(
        ...
        itemBuilder: (context, index) {
          ...
          final monthPlan =
              db.curUser.events.exchangeTickets.putIfAbsent(ticket.month, () => [0, 0, 0]);

          TextStyle plannedStyle;
          if (sum(db.curUser.events.exchangeTickets[ticket.month] ?? []) > 0) {
          ...
          if (index % 4 == 0) {
                ...
                child: Column(
                  ...
                  children: [
                    AutoSizeText(ticket.month,
                        maxLines: 1,
                        maxFontSize: 16,
                        style: plannedStyle.copyWith(fontWeight: FontWeight.bold)),
                    AutoSizeText(
                      '${ticket.monthJp}\nmax: ${ticket.days}',
                      maxLines: 2,
                      maxFontSize: 14,
            ...
            int leftNum = statistics.leftItems[iconKey] ?? 0;
            final maxValue = ticket.days - sum(monthPlan.getRange(0, itemIndex));
            return Center(
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: <Widget>[
                  ...
                  SizedBox(
                    ...
                    child: MaterialButton(
                      ...
                      child: Column(
                        mainAxisSize: MainAxisSize.min,
                        children: <Widget>[
                          Text(monthPlan[itemIndex] == 0 ? '' : monthPlan[itemIndex].toString()),
                          Divider(height: 1),
                          DefaultTextStyle(
                            style: Theme.of(context).textTheme.bodyText2, //body1->bodyText2
                            child: AutoSizeText(
                              ...
                              group: _autoSizeGroup,
                              style:
                                  TextStyle(color: leftNum >= 0 ? Colors.grey : Colors.redAccent),
                            ),
                          )
                        ],
                      ),
                      onPressed: () {
                        Picker(
                          ...
                          onConfirm: (picker, values) {
                            monthPlan[itemIndex] = picker.getSelectedValues()[0];
                            for (var j = 0; j < 3; j++) {
                              monthPlan[j] =
                                  min(monthPlan[j], ticket.days - sum(monthPlan.getRange(0, j)));
                            }
                            statistics.updateEventItems();
                          },
                          height: 400,
