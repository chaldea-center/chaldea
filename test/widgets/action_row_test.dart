...
void main() {
  ...
  testWidgets('renders leading, title, subtitle, chevron by default', (tester) async {
    await pumpRow(tester, const ActionRow(leading: Icon(Icons.settings), title: 'Settings', subtitle: 'Configure'));
  ...
  testWidgets('hides chevron when showChevron=false', (tester) async {
    await pumpRow(tester, const ActionRow(title: 'T', showChevron: false));
    expect(find.byIcon(Icons.chevron_right), findsNothing);
  });

  testWidgets('danger variant colors title with error', (tester) async {
    await pumpRow(tester, const ActionRow(title: 'Delete', variant: ActionRowVariant.danger));
  ...
  testWidgets('onTap invokes callback', (tester) async {
    var tapped = 0;
    await pumpRow(tester, ActionRow(title: 'T', onTap: () => tapped++));
